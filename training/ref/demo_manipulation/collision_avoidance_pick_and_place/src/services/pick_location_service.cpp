...
const std::string PICK_LOCATION_SERVICE = "target_recognition";
...
// ros parameter
std::string SENSOR_CLOUD_TOPIC = "/kinect/depth_registered/points";

// ros service server
ros::ServiceServer pick_location_server;
...
// function signatures
bool pick_location_callback(collision_avoidance_pick_and_place::GetTargetPose::Request& req,
...
bool get_pick_transform(tf::Transform& trg_to_src_tf);
...
{
	ros::init(argc,argv,"pick_location_node");
	ros::NodeHandle nh;
	ros::NodeHandle ph("~");
	...
	{
		ROS_INFO_STREAM("pick location read parameters successfully");
	}
	else
	{
		ROS_ERROR_STREAM("pick location did not find parameters");
	}

	// initializing service server
	pick_location_server = nh.advertiseService(PICK_LOCATION_SERVICE,pick_location_callback);
	...
	while(ros::ok())
	{
		...
		// read parameters
		ph.getParam("pick_x",PICK_X);
		ph.getParam("pick_y",PICK_Y);
		ph.getParam("pick_z",PICK_Z);
		ph.getParam("pick_roll",PICK_ROLL);
		ph.getParam("pick_pitch",PICK_PITCH);
		ph.getParam("pick_yaw",PICK_YAW);
		...
		collision_avoidance_pick_and_place::GetTargetPose::Response& res)
{
	...
	if(get_pick_transform(world_to_ar_tf) &&
...
bool get_transform(std::string target,std::string source,tf::Transform& trg_to_src_tf)
{
	...
	{
	  ROS_ERROR_STREAM("transform lookup for '"<<target<<" to "<<source<<"' failed");
		return false;
	}
...
bool get_pick_transform(tf::Transform& trg_to_src_tf)
{
	tf::Vector3 pick_position(PICK_X,PICK_Y,PICK_Z);
	tf::Transform T;
	trg_to_src_tf.setOrigin(pick_position);
	tf::Quaternion Q;
	Q.setEuler(PICK_YAW,PICK_PITCH,PICK_ROLL);
	trg_to_src_tf.setRotation(Q);
	T.setRotation(Q);
	return true;
}
